// NSEdgeInsetsExtensionsTests.swift - Copyright 2021 SwifterSwift
...
final class NSEdgeInsetsExtensionsTests: XCTestCase {
    func testHorizontal() {
        let inset = NSEdgeInsets(top: 30.0, left: 5.0, bottom: 5.0, right: 10.0)
        XCTAssertEqual(inset.horizontal, 15.0)
    }

    func testVertical() {
        let inset = NSEdgeInsets(top: 10.0, left: 10.0, bottom: 5.0, right: 10.0)
        XCTAssertEqual(inset.vertical, 15.0)
    }

    func testInitInset() {
        let inset = NSEdgeInsets(inset: 5.0)
    ...
    func testInitVerticalHorizontal() {
        let inset = NSEdgeInsets(horizontal: 20.0, vertical: 10.0)
        XCTAssertEqual(inset.top, 5.0)
        XCTAssertEqual(inset.bottom, 5.0)
    ...
    func testInsetByTop() {
        let inset = NSEdgeInsets(top: 10.0, left: 10.0, bottom: 10.0, right: 10.0)
    ...
    func testAddition() {
        XCTAssertEqual(NSEdgeInsets.zero + NSEdgeInsets.zero, NSEdgeInsets.zero)

        let insets1 = NSEdgeInsets(top: 1, left: 2, bottom: 3, right: 4)
        let insets2 = NSEdgeInsets(top: 5, left: 6, bottom: 7, right: 8)
        let expected = NSEdgeInsets(top: 6, left: 8, bottom: 10, right: 12)
        XCTAssertEqual(insets1 + insets2, expected)

        let negativeInsets1 = NSEdgeInsets(top: -1, left: -2, bottom: -3, right: -4)
        let negativeInsets2 = NSEdgeInsets(top: -5, left: -6, bottom: -7, right: -8)
        let negativeExpected = NSEdgeInsets(top: -6, left: -8, bottom: -10, right: -12)
        XCTAssertEqual(negativeInsets1 + negativeInsets2, negativeExpected)
    }

    func testInPlaceAddition() {
        var zero = NSEdgeInsets.zero
        zero += NSEdgeInsets.zero
        XCTAssertEqual(zero, NSEdgeInsets.zero)

        var insets = NSEdgeInsets(top: 1, left: 2, bottom: 3, right: 4)
        insets += NSEdgeInsets(top: 5, left: 6, bottom: 7, right: 8)
        let expected = NSEdgeInsets(top: 6, left: 8, bottom: 10, right: 12)
        XCTAssertEqual(insets, expected)

        var negativeInsets = NSEdgeInsets(top: -1, left: -2, bottom: -3, right: -4)
        negativeInsets += NSEdgeInsets(top: -5, left: -6, bottom: -7, right: -8)
        let negativeExpected = NSEdgeInsets(top: -6, left: -8, bottom: -10, right: -12)
